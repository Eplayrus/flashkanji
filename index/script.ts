(() => {
  ...
  const APP_VERSION = 3;
  const BUILD_VERSION = "2026-06-15-home-eva-v7";
...
if (await refreshStaleAppCache()) return;
  ...
  function handleClick(event) {
    ...
    if (["eva-click", "eva-autonomy-next", "eva-question-answer"].includes(action) && Date.now() - lastEvaDirectActionAt < 280) return;
  ...
  function clearCustomizationItem(itemId = null) {
    const normalizedId = normalizeCustomizationItemId(
      itemId ||
      state.customization?.selected?.effect ||
  ...
  function submitN4FinalTest(force = false) {
    ...
    try {
      ...
      let score = 0;
      const mistakes: Array<{ id: string; kanji: string; answer: string; selected: string }> = [];
      const unanswered: Array<ReturnType<typeof buildN4FinalQuestions>[number]> = [];
      questions.forEach((question) => {
        const selected = String(test.answers?.[question.id] || "").trim();
        if (selected === question.answer) {
          score += 1;
  ...
  function submitN3FinalTest(force = false) {
    ...
    try {
      ...
      let score = 0;
      const mistakes: Array<{ id: string; kanji: string; answer: string; selected: string }> = [];
      const unanswered: Array<ReturnType<typeof buildN3FinalQuestions>[number]> = [];
      questions.forEach((question) => {
        const selected = String(test.answers?.[question.id] || "").trim();
        if (selected === question.answer) {
          score += 1;
  ...
  function submitN2FinalTest(force = false) {
    ...
    try {
      ...
      let score = 0;
      const mistakes: Array<{ id: string; kanji: string; answer: string; selected: string }> = [];
      const unanswered: Array<ReturnType<typeof buildN2FinalQuestions>[number]> = [];
      questions.forEach((question) => {
        const selected = String(test.answers?.[question.id] || "").trim();
        if (selected === question.answer) {
          score += 1;
...
const APP_VERSION = '2026-06-12-n2-interactive-textbook-v1'; // Меняйте версию при каждом деплое
...
// Код ниже должен находиться внутри вашего sw.js:
  ...
  const CACHE_NAME = 'flash-kanji-cache-v90'; // Версия кэша должна совпадать с APP_VERSION
