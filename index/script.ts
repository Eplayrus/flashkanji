...
if (await refreshStaleAppCache()) return;
    setAppBooting(true);
    if (!app.innerHTML.trim()) app.innerHTML = renderLoading();
  ...
  function setAppBooting(isBooting) {
    ...
    if (shell) {
      if (isBooting) shell.setAttribute("data-booting", "true");
      else shell.removeAttribute("data-booting");
    }
    if (app) app.setAttribute("aria-busy", isBooting ? "true" : "false");
  }
  ...
  function startFlashKanjiOnboarding(options = {}) {
    ...
    try {
      ...
      if (shell) {
        ...
        try {
          (shell as HTMLElement).inert = true;
        } catch {}
      }
  ...
  function stopFlashKanjiOnboarding(options = {}) {
    ...
    if (shell) {
      ...
      try {
        (shell as HTMLElement).inert = false;
      } catch {}
    }
  ...
  function renderCharts() {
    ...
    if (!window.Chart) {
      loadChartLibrary()
        .then(() => {
          if (state.route === "stats") renderCharts();
        })
  ...
  function romajiForReadingPair(kana, romaji) {
    ...
    if (!expected) return romaji || "";
    if (!romaji) return expected;
